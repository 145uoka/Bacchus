...
public class EventEditController extends BaseController {
	...
	public String eventEdit(@ModelAttribute("form") EventEditForm form, BindingResult bindingResult,
		...
		for (int i = 0; i < candidateTList.size(); i++) {
			CandidateDto candidateDto = eventService.convCandidateEntityToDto(candidateTList.get(i));
			candidateDtoList.add(candidateDto.getStartDate());

			// ラジオボタンの初期表示
			if (candidateDto.getCandidateNo().equals(eventDto.getCandidateNo())) {

 ...
 */
public class BsEntryTCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected EntryTCQ _conditionQuery;
    ...
    public EntryTCB acceptPK(Long entryId) {
        ...
        return (EntryTCB)this;
    }

    public ConditionBean addOrderBy_PK_Asc() {
        query().addOrderBy_EntryId_Asc();
        return this;
    }

    public ConditionBean addOrderBy_PK_Desc() {
        query().addOrderBy_EntryId_Desc();
        return this;
    }
     ...
     */
    public EntryTCQ query() {
        assertQueryPurpose(); // assert only when user-public query 
        return doGetConditionQuery();
    }

    public EntryTCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected EntryTCQ doGetConditionQuery() {
    ...
    protected EntryTCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected EntryTCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        EntryTCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected EntryTCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new EntryTCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<EntryTCB> unionCBLambda) {
        final EntryTCB cb = new EntryTCB(); cb.xsetupForUnion(this); xsyncUQ(cb); 
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final EntryTCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<EntryTCB> unionCBLambda) {
        final EntryTCB cb = new EntryTCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final EntryTCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<EntryTCQ> {
         ...
         */
        public HpSDRFunction<EntryTCB, EntryTCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<EntryTCB> sq, EntryTCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public EntryTCB dreamCruiseCB() {
        EntryTCB cb = new EntryTCB();
        cb.xsetupForDreamCruise((EntryTCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<EntryTCB> columnQuery(final SpecifyQuery<EntryTCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected EntryTCB xcreateColumnQueryCB() {
        EntryTCB cb = new EntryTCB();
        cb.xsetupForColumnQuery((EntryTCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<EntryTCB> orCBLambda) {
        xorSQ((EntryTCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<EntryTCB> andCBLambda) {
        xorSQAP((EntryTCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return EntryTCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return EntryTCQ.class.getName(); }

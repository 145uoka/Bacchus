...
<body>
	...
	<form:form modelAttribute="form" name="form"
		action="${pageContext.request.contextPath}/entry/create" method="get"
										...
										<table
											...
											<thead class="main-background-color">
												<tr>
													...
													<c:forEach var="list" items="${candidateDtoList}"
														varStatus="status">
														<th class="text-center"><c:out
																value="${list.eventStartDatetimeDisplay}" /></th>
													</c:forEach>
												</tr>
											</thead>
											<tbody>
												<tr>
													...
													<c:forEach var="entrySummary" items="${entrySummaryList}"
														varStatus="status">
														<td class="text-center"><c:out
																value="${entrySummary}" /></td>
													</c:forEach>
												</tr>
												<c:forEach var="list" items="${entryDispListDtoList}"
													varStatus="status">
													<c:choose>
														<c:when test="${userInfo.adminFlg}">
															<tr
																data-href="${pageContext.request.contextPath}/entry/create?eventNo=${form.eventNo}&userId=${list.userId}">
														</c:when>
														<c:otherwise>
															<tr>
														</c:otherwise>
														...
														<td class="text-center"><c:choose>
																<c:when test="${entryDtoList.entryDiv == 1}">
																	<span class="label label-info"
																		style="margin-left: 10px"> <c:out
																			value="${entryDtoList.entryDivDisp}" />
																	</span>
																</c:when>
